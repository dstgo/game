// Code generated by protoc-gen-go-http. DO NOT EDIT.
// versions:
// - protoc-gen-go-http v2.7.3
// - protoc             v5.29.2
// source: api/v1/world/game_world_service.proto

package world

import (
	context "context"
	http "github.com/go-kratos/kratos/v2/transport/http"
	binding "github.com/go-kratos/kratos/v2/transport/http/binding"
	emptypb "google.golang.org/protobuf/types/known/emptypb"
)

// This is a compile-time assertion to ensure that this generated file
// is compatible with the kratos package it is being compiled against.
var _ = new(context.Context)
var _ = binding.EncodeURL

const _ = http.SupportPackageIsVersion1

const OperationWorldHello = "/api.v1.world.World/Hello"

type WorldHTTPServer interface {
	Hello(context.Context, *emptypb.Empty) (*emptypb.Empty, error)
}

func RegisterWorldHTTPServer(s *http.Server, srv WorldHTTPServer) {
	r := s.Route("/")
	r.GET("/game/v1/lobby/hello", _World_Hello3_HTTP_Handler(srv))
}

func _World_Hello3_HTTP_Handler(srv WorldHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in emptypb.Empty
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationWorldHello)
		h := ctx.Middleware(func(ctx context.Context, req any) (any, error) {
			return srv.Hello(ctx, req.(*emptypb.Empty))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*emptypb.Empty)
		return ctx.Result(200, reply)
	}
}

type WorldHTTPClient interface {
	Hello(ctx context.Context, req *emptypb.Empty, opts ...http.CallOption) (rsp *emptypb.Empty, err error)
}

type WorldHTTPClientImpl struct {
	cc *http.Client
}

func NewWorldHTTPClient(client *http.Client) WorldHTTPClient {
	return &WorldHTTPClientImpl{client}
}

func (c *WorldHTTPClientImpl) Hello(ctx context.Context, in *emptypb.Empty, opts ...http.CallOption) (*emptypb.Empty, error) {
	var out emptypb.Empty
	pattern := "/game/v1/lobby/hello"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationWorldHello))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
	}
	return &out, err
}
