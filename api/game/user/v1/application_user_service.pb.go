// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.34.1
// 	protoc        v4.24.4
// source: api/game/user/application_user_service.proto

package v1

import (
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_api_application_user_application_user_service_proto protoreflect.FileDescriptor

var file_api_application_user_application_user_service_proto_rawDesc = []byte{
	,
}

var file_api_application_user_application_user_service_proto_goTypes = []interface{}{
	(*GetCurrentUserRequest)(nil),    // 0: game.api.game.user.v1.GetCurrentUserRequest
	(*UpdateCurrentUserRequest)(nil), // 1: game.api.game.user.v1.UpdateCurrentUserRequest
	(*GetUserRequest)(nil),           // 2: game.api.game.user.v1.GetUserRequest
	(*ListUserRequest)(nil),          // 3: game.api.game.user.v1.ListUserRequest
	(*CreateUserRequest)(nil),        // 4: game.api.game.user.v1.CreateUserRequest
	(*ImportUserRequest)(nil),        // 5: game.api.game.user.v1.ImportUserRequest
	(*ExportUserRequest)(nil),        // 6: game.api.game.user.v1.ExportUserRequest
	(*UpdateUserRequest)(nil),        // 7: game.api.game.user.v1.UpdateUserRequest
	(*UpdateUserStatusRequest)(nil),  // 8: game.api.game.user.v1.UpdateUserStatusRequest
	(*DeleteUserRequest)(nil),        // 9: game.api.game.user.v1.DeleteUserRequest
	(*GetTrashUserRequest)(nil),      // 10: game.api.game.user.v1.GetTrashUserRequest
	(*ListTrashUserRequest)(nil),     // 11: game.api.game.user.v1.ListTrashUserRequest
	(*DeleteTrashUserRequest)(nil),   // 12: game.api.game.user.v1.DeleteTrashUserRequest
	(*RevertTrashUserRequest)(nil),   // 13: game.api.game.user.v1.RevertTrashUserRequest
	(*GetCurrentUserReply)(nil),      // 14: game.api.game.user.v1.GetCurrentUserReply
	(*UpdateCurrentUserReply)(nil),   // 15: game.api.game.user.v1.UpdateCurrentUserReply
	(*GetUserReply)(nil),             // 16: game.api.game.user.v1.GetUserReply
	(*ListUserReply)(nil),            // 17: game.api.game.user.v1.ListUserReply
	(*CreateUserReply)(nil),          // 18: game.api.game.user.v1.CreateUserReply
	(*ImportUserReply)(nil),          // 19: game.api.game.user.v1.ImportUserReply
	(*ExportUserReply)(nil),          // 20: game.api.game.user.v1.ExportUserReply
	(*UpdateUserReply)(nil),          // 21: game.api.game.user.v1.UpdateUserReply
	(*UpdateUserStatusReply)(nil),    // 22: game.api.game.user.v1.UpdateUserStatusReply
	(*DeleteUserReply)(nil),          // 23: game.api.game.user.v1.DeleteUserReply
	(*GetTrashUserReply)(nil),        // 24: game.api.game.user.v1.GetTrashUserReply
	(*ListTrashUserReply)(nil),       // 25: game.api.game.user.v1.ListTrashUserReply
	(*DeleteTrashUserReply)(nil),     // 26: game.api.game.user.v1.DeleteTrashUserReply
	(*RevertTrashUserReply)(nil),     // 27: game.api.game.user.v1.RevertTrashUserReply
}
var file_api_application_user_application_user_service_proto_depIdxs = []int32{
	0,  // 0: game.api.game.user.v1.User.GetCurrentUser:input_type -> game.api.game.user.v1.GetCurrentUserRequest
	1,  // 1: game.api.game.user.v1.User.UpdateCurrentUser:input_type -> game.api.game.user.v1.UpdateCurrentUserRequest
	2,  // 2: game.api.game.user.v1.User.GetUser:input_type -> game.api.game.user.v1.GetUserRequest
	3,  // 3: game.api.game.user.v1.User.ListUser:input_type -> game.api.game.user.v1.ListUserRequest
	4,  // 4: game.api.game.user.v1.User.CreateUser:input_type -> game.api.game.user.v1.CreateUserRequest
	5,  // 5: game.api.game.user.v1.User.ImportUser:input_type -> game.api.game.user.v1.ImportUserRequest
	6,  // 6: game.api.game.user.v1.User.ExportUser:input_type -> game.api.game.user.v1.ExportUserRequest
	7,  // 7: game.api.game.user.v1.User.UpdateUser:input_type -> game.api.game.user.v1.UpdateUserRequest
	8,  // 8: game.api.game.user.v1.User.UpdateUserStatus:input_type -> game.api.game.user.v1.UpdateUserStatusRequest
	9,  // 9: game.api.game.user.v1.User.DeleteUser:input_type -> game.api.game.user.v1.DeleteUserRequest
	10, // 10: game.api.game.user.v1.User.GetTrashUser:input_type -> game.api.game.user.v1.GetTrashUserRequest
	11, // 11: game.api.game.user.v1.User.ListTrashUser:input_type -> game.api.game.user.v1.ListTrashUserRequest
	12, // 12: game.api.game.user.v1.User.DeleteTrashUser:input_type -> game.api.game.user.v1.DeleteTrashUserRequest
	13, // 13: game.api.game.user.v1.User.RevertTrashUser:input_type -> game.api.game.user.v1.RevertTrashUserRequest
	14, // 14: game.api.game.user.v1.User.GetCurrentUser:output_type -> game.api.game.user.v1.GetCurrentUserReply
	15, // 15: game.api.game.user.v1.User.UpdateCurrentUser:output_type -> game.api.game.user.v1.UpdateCurrentUserReply
	16, // 16: game.api.game.user.v1.User.GetUser:output_type -> game.api.game.user.v1.GetUserReply
	17, // 17: game.api.game.user.v1.User.ListUser:output_type -> game.api.game.user.v1.ListUserReply
	18, // 18: game.api.game.user.v1.User.CreateUser:output_type -> game.api.game.user.v1.CreateUserReply
	19, // 19: game.api.game.user.v1.User.ImportUser:output_type -> game.api.game.user.v1.ImportUserReply
	20, // 20: game.api.game.user.v1.User.ExportUser:output_type -> game.api.game.user.v1.ExportUserReply
	21, // 21: game.api.game.user.v1.User.UpdateUser:output_type -> game.api.game.user.v1.UpdateUserReply
	22, // 22: game.api.game.user.v1.User.UpdateUserStatus:output_type -> game.api.game.user.v1.UpdateUserStatusReply
	23, // 23: game.api.game.user.v1.User.DeleteUser:output_type -> game.api.game.user.v1.DeleteUserReply
	24, // 24: game.api.game.user.v1.User.GetTrashUser:output_type -> game.api.game.user.v1.GetTrashUserReply
	25, // 25: game.api.game.user.v1.User.ListTrashUser:output_type -> game.api.game.user.v1.ListTrashUserReply
	26, // 26: game.api.game.user.v1.User.DeleteTrashUser:output_type -> game.api.game.user.v1.DeleteTrashUserReply
	27, // 27: game.api.game.user.v1.User.RevertTrashUser:output_type -> game.api.game.user.v1.RevertTrashUserReply
	14, // [14:28] is the sub-list for method output_type
	0,  // [0:14] is the sub-list for method input_type
	0,  // [0:0] is the sub-list for extension type_name
	0,  // [0:0] is the sub-list for extension extendee
	0,  // [0:0] is the sub-list for field type_name
}

func init() { file_api_application_user_application_user_service_proto_init() }
func file_api_application_user_application_user_service_proto_init() {
	if File_api_application_user_application_user_service_proto != nil {
		return
	}
	file_api_application_user_application_user_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_api_application_user_application_user_service_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_api_application_user_application_user_service_proto_goTypes,
		DependencyIndexes: file_api_application_user_application_user_service_proto_depIdxs,
	}.Build()
	File_api_application_user_application_user_service_proto = out.File
	file_api_application_user_application_user_service_proto_rawDesc = nil
	file_api_application_user_application_user_service_proto_goTypes = nil
	file_api_application_user_application_user_service_proto_depIdxs = nil
}
